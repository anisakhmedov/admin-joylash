    ...
    <div id="userCreated" class="container">
        ...
        <div class="house-card">
            <div class="house-main">
                <img :src="user.mainImage" alt="Главное изображение" class="main-image"
                ...
                <div class="house-info">
                    ...
                    <div class="details">
                        ...
                        <p><strong>Продажа / Аренда:</strong> {{ user.RentOrSell == 1 ? 'Продажа' : 'Аренда' }}</p>
                        <p><strong>Дата:</strong> {{ user.data }}</p>
                        <p><strong>Номер телефона:</strong> <a :href="'tel:' + user.phoneNumberUser">{{
                            user.phoneNumberUser }}</a></p>
                        <p><strong>Проверен модератором:</strong> {{ user.isModerate ? 'Да' : 'Нет' }}</p>
                    </div>

                    <div class="pluses">
                        <h3>Преимущества:</h3>
                        <ul>
                            <li v-for="(plus, index) in user.pluses" :key="index">{{ plus }}</li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            <div class="gallery">
                ...
                <div class="gallery-images">
                    <img v-for="(img, idx) in user.additionalImages" :key="idx" :src="img" alt="Изображение"
                        @click="openModal(img)" />
                </div>
            </div>
        </div>
...
export default {
    ...
    async mounted() {
        ...
        const res = await axios.get('https://joylash-uz-4a09707016fe.herokuapp.com/houses/' + this.linkHouse)
